...
- **Each modem's label says whether the node is attached** - `1 host` / `no host`, over both its dedicated KISS port and the multiplexed one, following clients in and out. A host that quietly dropped its TCP session looks, from the modem's side, exactly like a band that went quiet.
