...
**Status: retired, 2026-09-03**, in favour of https://monitor.ukpacketradio.network, which shows M9PSY-1 among many other receivers rather than on its own (see [docs/monitor-plan.md](monitor-plan.md)). It shipped in v0.54.0 (PR #382) and ran live at https://m9psy-1-monitor.ukpacketradio.network from CT 146 for part of one day. CT 146 is repurposed as the new deployment, its tunnel renamed, and the `m9psy-1-monitor` hostname removed from DNS; the frame history the site collected was carried over as `/var/lib/pdn-soundmodem/frames-m9psy-1.db`, which is what the new flavour calls that receiver's log, so the receiver's page kept it. The rest of this document is left as the record of what was built and why. Written at Tom's request for a public-facing deployment of pdn-soundmodem at https://m9psy-1-monitor.ukpacketradio.network, showing the 7050-7052 kHz packet window that GB7RDG-2 works, as heard by a public UberSDR receiver.
