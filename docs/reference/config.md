...
- **A radio's answer is the station's; the audio answer is each sub-channel's.** With no radio configured, a frame waits only for its own modem's detector and for any modem whose passband overlaps it, rather than for every modem on the station. That matters on a station whose modems sit on different RF frequencies inside one passband: GB7RDG's five sub-channels were individually busy 35 to 81 % of one 228 second stretch and their union was busy 96.8 %, so the modem that had the traffic saw one usable gap where its own channel had fourteen. Overlap is measured from each modem's occupied bandwidth, so modems a couple of hundred Hz apart still defer to each other; see [carrier-sense.md](../dev/carrier-sense.md#what-526-changed-who-the-answer-is-for).
