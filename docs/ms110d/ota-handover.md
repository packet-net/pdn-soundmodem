...
| §E2 hardware-in-the-loop | **done 2026-07-27** over the pad-chain rig, not the dummy-load path this document was written for: 18 live ladders, AWGN WN0-8/13 + Poor WN0-6/13, every masked waveform at or below its mask, 0 run failures ([evidence/2026-07-27-110d-full-campaign/](evidence/2026-07-27-110d-full-campaign/)); the two real-RF Poor failures diagnosed and fixed the same day ([evidence/2026-07-27-ota-poor-validation/](evidence/2026-07-27-ota-poor-validation/)). WN7/WN8 Poor not run - above the rig ceiling |
| §E3 IQ vs SSB A/B | **not run** as a seeded pair. Both routes exist (`--route iq|dax`); the only DAX datum is the lab campaign's ALC finding (drive 0.9 costs ~1.2 dB, back off to ~0.5-0.7). Backlog A12 |
| §E4 on air | **done 2026-08-03** from a real antenna at 48 W into wessex: WN0-7 and WN13 bit-exact, WN8 the ceiling of the path ([evidence/2026-08-03-e4-first-on-air/](evidence/2026-08-03-e4-first-on-air/)). Single bursts per waveform; the hour-long, power-varied Phase 1 ladder of [ota-test-plan.md](ota-test-plan.md) and the m9psy second site are still owed (backlog A10) |
...
- **Uncoded BER grades opinions, not erasures.** An exactly-zero LLR is the soft-output path expressing no opinion - WN0 erases its first channel symbol of every burst by design, because the RAKE's decision-directed finger gains start cold - so those positions are neither bits compared nor errors. The hard-decision tie-break reads an erasure as a bit 1, so grading it would charge up to 2/80 = 2.5 % on a clean WN0 Short block regardless of SNR, and every clean-channel point would sit above the curve it is compared with. The excluded count is reported per burst (`UncodedErasures` on the score record, the `uncodedErasures` CSV column, and the pooled summary line), so an LLR path gone quiet shows up as a growing count instead of silently flattering the rate - WN0 reports a structural 2 per burst, every other waveform 0.
