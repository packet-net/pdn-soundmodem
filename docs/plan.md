...
### 2026-08-21 (later3) - timing diversity: three decision phases per symbol for every PSK mode, another ~0.7-1 dB at four knees

Tom asked why the receiver could not do what the bench had done to the #326 fixture, which was to try the clock phases and keep the one that passed. Now it does, in miniature: `TimingDiversity` gives both PSK demodulators three decision phases per symbol (the recovered instant and 5 % of a symbol either side, interpolated so every samples-per-symbol ratio works), each with its own reference state and its own `Il2pReceiver`, and the modem delivers whichever copy passes, once, behind a symbol-clocked dedupe shorter than any frame. The filters, clock and offset window are shared, so the cost is the decision stage and two deframers per branch. Measured on deterministic seeds (N=200): qpsk600 -1/0 dB 100/166 -> 131/185, qpsk2400 +6/+7 dB 155/180 -> 184/192, bpsk300 -5/-4 dB 133/184 -> 164/195, bpsk1200 +1/+2 dB 131/186 -> 160/197; CFO rows hold or improve; qpsk3600 gains a few points with its detector unchanged. Two faults found by the measurement and fixed: the first cut's phases were [0, -0, 0] through a static-initialiser ordering slip (caught by per-phase fixture scoring, not by the ladder, which read a null), and the QPSK rotation tracker was walking on noise between bursts to its clamp and losing short-preamble bursts that started there (a latent #328 weakness; the tracker now integrates only while a signal is present and restarts at a burst's first energy, while BPSK's stays free-running because gating it measurably cost the selection diversity #236 recorded). The fixture still does not copy. Full numbers in docs/mode-validation.md (2026-08-21, later4).
