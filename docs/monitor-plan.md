...
**Status: planned, 2026-09-03.** The successor to [docs/40m-monitor-plan.md](40m-monitor-plan.md), which shipped in v0.54.0/v0.54.1 and is live at https://m9psy-1-monitor.ukpacketradio.network from CT 146 on proxmox1, one receiver per deployment. Tom has asked for the bigger version: one deployment at https://monitor.ukpacketradio.network with a front page that lists the UberSDR receivers from the public directory, a visitor picks one, and the daemon holds at most one session on each receiver however many people are watching it. The single-receiver flavour does not go away.
...
## 5. Deployment (Phase 4, sketch only)

Decisions here are Tom's and are not taken in this document.

- **Container**: a new unprivileged LXC `monitor` on proxmox1 (`root@10.45.0.10`), following CT 146's shape exactly: Debian 13, `keyctl=1,nesting=1`, `net0` on `vmbr0` by DHCP, rootfs on `local-zfs`, `swap: 512`, `onboot: 1`, `unattended-upgrades` over Debian security and the cloudflared repo.
...
- **Memory**: an idle single-receiver daemon measured 118 MB in CT 146. Each additional idle station in-process is expected to be a few MB. Measure it in Phase 3 with two, five and ten stations created and write the real number back into this section; the container's RAM allocation depends on it and a guess is not good enough for fifty.
- **Open question for Tom**: whether CT 146 and `m9psy-1-monitor.ukpacketradio.network` stay up alongside the new deployment. Keeping it costs one container and proves the two-flavour claim in production; retiring it means one site to explain. Nothing in this design forces either.
...
- [ ] **Phase 2** - `WaterfallWebServer` under a path base, page URLs relative, the two-server router. Can run beside Phase 1 in its own worktree. PR:
