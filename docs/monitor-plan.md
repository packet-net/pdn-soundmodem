...
- One row per receiver a visitor can pick: callsign, name, location, `snr_0_30_mhz` as a signal figure where the directory gives one, free slots as `available_clients` of `max_clients`, and a link to `public_url` for the receiver's own page. The row itself links to `/r/<slug>/`. There is no column for what the receiver is doing and no viewer count in the row; a row somebody is watching keeps its faint tint, and the number is in the count across the top. See section 8, 2026-09-04.
...
Taken as: the column, the header cell over it, the live-first order it was the only explanation for, and the rows nobody can act on all go; every row that is left is a link. This supersedes the picker half of "Filtered means not listed; full means listed and not offered" below, and of three others there that quote a sentence the picker used to put in a row - the retry line, the fault line, the daily allowance line. `/api/instances` is untouched and still carries `state`, `viewers`, `why` and `status`; the receiver's own page still says all of it in its status chip, which is the only place a visitor who has picked one will be looking. What is lost from the picker, and worth naming rather than discovering: a visitor can no longer see from the front page that a receiver's daily allowance for this monitor is spent, or that a station is not working just now - they see it on the receiver's page, one click away.
