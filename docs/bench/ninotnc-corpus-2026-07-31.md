...
Every file was energy-segmented (all 3 bursts physically present) and decoded through pdn-soundmodem's paired catalog mode (`NinoCorpusQcTests`, report `nino-qc-final.txt`): **43/45 decode 3/3** (final, after the capture-glitch remediation below) (as of the 2026-07-31 QPSK detector default reversal — differential detection copies all nine QPSK files; under the earlier coherent default `qpsk600` copied 0–1/3 and `qpsk3600` 1–2/3, the diagnosis that drove the reversal). The 2 remaining exceptions are pdn-soundmodem limitations on good captures, kept deliberately as interop test material: `c4fsk9600`'s acquisition band — 50 ms TXDELAY is below its hard floor (0/3), 120 ms is marginal (2/3, repeatable across captures), ≥250 ms reliable (3/3).
