...
[`docs/dev/plan.md`](plan.md) (the phase plan + §17 amendment log) and
[`docs/dev/waveform-roadmap.md`](waveform-roadmap.md) (the approved modem build order and the
...
validation remains**. Procedure written: [`docs/dev/archive/freedv-hf-loop.md`](archive/freedv-hf-loop.md) - the 8
...
statistical budget, KISS-integrated (`ms110d-wn*`); `docs/dev/archive/ms110d/phase-a-closeout.md`.
- **Phase B closed 2026-07-27** (`docs/dev/archive/ms110d/phase-b-closeout.md`): 8PSK (WN7) / 16QAM (WN8)
...
- **WN8 redesign program closed 2026-07-31, exit (ii)** (`docs/dev/archive/ms110d/wn8-program-plan.md`):
...
- **Poor-gate successor program registered 2026-08-20**
  (`docs/dev/archive/ms110d/poor-gate-successor-plan.md`, issue #312): G0 bank + re-baseline done; **G1d
...
- On-air (2026-07-27/28): every masked waveform meets its mask over the pad-chain rig
  (`docs/dev/archive/ms110d/evidence/2026-07-27-110d-full-campaign/`). **§E4 2026-08-03**: first
  transmissions from a real antenna at 48 W, WN0-7 and WN13 bit-exact into wessex, WN8 the
  ceiling of that path (`docs/dev/archive/ms110d/evidence/2026-08-03-e4-first-on-air/`). WN7/WN8 Poor
...
and a listening correspondent unattended. Plan: [`docs/dev/plans/ale.md`](plans/ale.md). Prompted by the
...
- **IQ interfaces (Tom's prompt) - researched + TX proven, 2026-07-17** (see
  [flex-integration.md §9](archive/flex-integration.md)): **RX** = DAX-IQ, wideband complex baseband, but
...
### #17 - Mixer, AGC and mic boost controls for CM108 sound cards *(delivered, PR #416; amended by PR #419 and PR #424)*
...
Amended again 2026-09-06 at Tom's request ("just add an enableAudioControls config setting"): `"waterfall": { "enableAudioControls": true }` puts the Mixer group on the operator's page and answers `/api/mixer` with no `api.key` at all, for a station whose page only its operator can reach - the bench station radio1 has no key, so the group was hidden and the endpoint a 404 there. Off by default, one endpoint only, never on a public page, and a keyless POST carrying a foreign `Origin` is refused as the TX test's is.

Amended again 2026-09-06, Tom running it on the bench Pi with the page open: "I don't see a TX Gain control. AGC should just be forced off, as should mic boost. No need for buttons for these. Some assistance in setting the capture level would be useful. No way for the user to know what good means." Three changes, all in PR #424. The Mixer group gets a second slider, **TX gain**, on the card's playback control, with the same rules as the capture one (the card's own dB range, the mute step under a CM108 Speaker's bottom excluded and said in the tooltip, refused rather than clamped, remembered in the state file, `?persist=false` for a one-run try). **AGC and mic boost stop being settings**: on any card that has them they are switched off at every start-up, whatever they were and whatever the file says, in one journal line; the two buttons are gone from the page, the `agc`/`micBoost` keys are refused by name in the config file (a warning, not an exit; the sentence takes the value, because "the card ends up the way it asked for either way" is true of `false` and a lie about `true`) and with a 400 over the API, and the state file has no field for them. `GET /api/mixer` still reports both read-only as `{"control": ..., "on": ..., "forcedOff": true}`, deliberately: it is the only way to see that a card has an AGC and refused to switch it off, which a null would make indistinguishable from the ordinary CM108 that simply has no mic boost. And an **input level meter** goes beside the capture slider - peak and RMS in dBFS measured at the daemon's input, after the card and before any modem, sent over the page's own WebSocket five times a second, free when no browser has the page open (the meter is reset rather than accumulated), and offered only where the page will actually carry the Mixer group: an operator's page, a sound card, and an `api.key` or `enableAudioControls`.

Review of the first cut (PR #424, 2026-09-06) found the meter's numbers wrong on the bench and they are worth recording, because the shape of the mistake will recur. Peak and RMS come off the channel's receive tap, which on a 48 kHz card is the **decimator's output**; the 48 to 12 kHz FIR's ripple moves peaks either way, up to about 1.3 dB above the card's own peak on radio1, so the probe read peaks of **+0.8 dBFS** - a level no 16-bit code can hold - and lit CLIP on 5 intervals out of 17 on a signal that never railed. Full scale is a fact about the converter and can only be measured before the resampler, so the clip count now comes from a card-rate tap in `Station` (`StationOptions.CardRateTap` -> `WaterfallWebServer.MeterInputClipping`), counting only exact top and bottom codes, and the reported peak and RMS are clamped to 0 dBFS. The same run showed the meter running at **3.4 messages a second** rather than five: `TryTake` restarted its interval from the moment of the take rather than from the boundary it crossed, and since the boundary is only tested when a 100 ms block arrives, one block a hair short locks it at 300 ms spacing for ever. It advances by whole intervals now. Neither fault was reachable by the existing tests, which fed `ProcessReceive` directly and advanced a `FakeTimeProvider` by exactly one interval; both have tests on the real path now.

The meter's target zone is **-18 to -9 dBFS on received-signal peaks**, and it is derived from this repository rather than borrowed: the bench NinoTNC loop's GOOD band is 0.17 to 0.28 full scale = -15.4 to -11.1 dBFS (docs/dev/bench/ninotnc-loop.md), a CFO campaign exonerated its recordings at 0.18 to 0.25 (docs/dev/archive/cfo/evidence/2026-07-31-cfo-1-qpsk-differential), the CM108 interface is designed for -12 dBFS at 60% of class deviation (docs/dev/hardware/tm8100-cm108-interface-notes.md), and an over-the-air capture called "comfortably adequate" peaked at -11.7 dBFS with 12 dB of headroom (docs/dev/archive/ms110d/evidence/2026-07-24-ota-c0). Widened a little either side because a real station's bursts vary and a zone the signal flickers out of is one an operator learns to ignore. Red above -3 dBFS (NinoBench's own CLIPPING verdict is -0.9 dBFS and the house receive reserve is about 6 dB of headroom), grey below -30 dBFS - advice rather than a fault, because every demodulator here is level-tolerant and the AFSK discriminator is barely affected from -40 dBFS up. Amended 2026-09-07, Tom: "the two thresholds should be determined by examining the modems and determining what the desired/optimal levels are" - so they were, in **[docs/dev/receive-levels.md](receive-levels.md)**, which reads every demodulator's front end for absolute-level dependencies and then decodes real frames at every level from 24 dB past full scale down to the converter's own floor. The zone survives; the red edge moves to -6 dBFS, which is the strictest mode's own headroom requirement; and the per-frame badges become the mode's own two numbers rather than the meter's, because the catalogue splits into three groups that differ by 6 dB at the loud end and 39 at the quiet one. **Still needs Tom's bench:** the TX slider heard on a real transmission, the meter read against a real signal on the air rather than a generated tone, and a card that genuinely has a Mic Boost control to be switched off.

Bench finding, radio1 2026-09-06, worth keeping because it will recur on other cards: the CM108's `Speaker Playback Volume` carries the TLV mute flag (`dBminmaxmute-min=-37.00dB,max=0.00dB`), so `snd_tlv_get_dB_range` reports its range minimum as `SND_CTL_TLV_DB_GAIN_MUTE`, the sentinel -9999999 in hundredths of a dB. Taken as a level that is "-99999.99 dB" in the journal and a slider a thousand times too long to aim a transmit level with, and it makes the range check accept a value that silently mutes the transmitter. Any card whose bottom step is its mute behaves this way, so the sentinel has to be recognised rather than passed through: the usable bottom is the lowest raw step that `snd_mixer_selem_ask_*_vol_dB` calls a level, and what is under it is said in words.

**Capture-start race - a rule for anyone touching start-up: all mixer work must finish before the PCM is opened.** Measured on radio1, 2026-09-06, with the mixer pass inside the window: 10 runs dead out of 13, against 12 of 12 alive for the same source doing two fewer reads per control; with the pass moved ahead of `snd_pcm_open`, 10 of 10 alive. Keep the rule: open the mixer, apply, read back, and be finished before `AlsaAudioOutput` and `AlsaAudioInput` are constructed. Nothing in the mixer pass needs the PCM (`--mixer-show` reads a card on a running station without touching it), so it costs nothing.

The mechanism first written here - a mixer control transfer colliding with the URB submission the first `snd_pcm_readi` makes - was a guess from the symptom, and the strace of the qpsk3600 failure the same evening says otherwise. The stream was started at `snd_pcm_open` with a 120 ms buffer, so it was free-running before the receive loop was ready: first read 80 ms after the start and fine, second read 150 ms later, `-EPIPE`. That is an overrun, and the recovery then prepared and read again 5 ms later without starting the stream, got `-EIO` from a card that had not finished stopping, and the daemon called the device dead. Anything that delays the first reads gets there: a mixer pass in the window, or, with the window empty, the first pass through a qpsk3600 receive chain being JIT-compiled on a Pi. afsk1200 survived because its first pass is shorter, which is exactly how a latent hazard looks. The reorder shortened the gap and did not remove it; a 500 ms capture buffer, a start threshold of 1 so the stream starts on the first read, and a recovery that prepares, starts, pauses and retries do (`AlsaPcm`, `PcmTransfer`).

**Capture set-up, benched on the CM108 and owed on every other card (PR #422).** That fix changes how EVERY ALSA station opens its capture stream, the live node included, so what has been proven and what has not is worth stating. Proven on radio1, 2026-09-06, with the branch build: `qpsk3600` alive at 8 s on 10 starts of 10 where every start died before, `afsk1200` 10 of 10 as well, no overruns in either set, and the card negotiated `buffer_size: 24000` (500 ms exactly at 48 kHz), `period_size: 1440` (30 ms) and `start_threshold: 1`, with the daemon's own line reading `audio: capture buffer 500 ms, period 30 ms`. **Still needs Tom's bench:** one start-up on GB7RDG's own card, which is a different card on a station with a transmitter and has only ever run the 120 ms configuration; and, on any card that refuses the request, the fallback path and the line that now names the errno (nothing has exercised it, because nothing has yet refused).

Amended in PR #419, and proven on the bench CM108 the same day across all eleven blocks of the bench script: levels in dB against the card's own range everywhere the value is shown; the two percentage keys gone rather than aliased, each refused by name; a change from the page or `POST /api/mixer` persisted by default in `mixer-state.json` (config file first, then state file, then the card left alone), with the config file never written and proven byte-identical through a persisted change; `?persist=false` as the one-run try; and the ordering rule above. **Still needs Tom's bench:** the page's Mixer group driven from a real browser (the endpoint and the page's own script are covered, a human at the slider is not), a `playbackDb` change heard on an actual transmission, a card that genuinely has a Mic Boost control, and a CM108 unplugged while the daemon holds its mixer open.

Shipped in PR #416, and proven on the bench CM108 (0d8c:0012) rather than only in tests: an `alsa.mixer` block (capture gain and playback level as percentages of the card's range, `agc`, `micBoost`, optional per-setting control-name lists), applied through `snd_mixer_*` at start-up and journalled as set and as read back; `GET`/`POST /api/mixer` under the config API's key, applied live with no restart; a Mixer group beside the display levels on the operator page and never on the public one; and `--mixer-show DEVICE`, which reads a card without touching the PCM. Absent keys change nothing, so every existing deployment is untouched. The bench revision has no "Mic Boost" control at all, which is what exercised the not-found path for real, and it found two bugs that are fixed here: a mixer change could not be folded into a config file with comments in it (the same defect was in `ProposedConfig`), and a POST could set the card and then abort the connection when `--config` named a file with no directory. Still needed Tom's bench at that point: the page group driven from a real browser, `playbackPercent` heard on a transmission, a card that actually has a Mic Boost, and a CM108 unplugged while the daemon holds its mixer open. (That list is superseded by PR #419's above; `playbackPercent` no longer exists.)
...
Also under this item, for FM radios, a Bessel null deviation check (Tom, 2026-09-05, "wonder about adding bessel null testing for FM actually, in that same roadmap item ... a 999hz tone will drive a bessel null at 2.4khz deviation. a 500hz tone will drive a bessel null at 1.2khz deviation. a 2079hz tone will drive a bessel null at 5khz deviation. a 1248hz tone will drive a bessel null at 3khz deviation. This would be useful. Don't do it now."). The rule behind those four pairs: the carrier of an FM signal vanishes when the modulation index is 2.405 (the first zero of the Bessel function J0), so with a single tone at f the carrier nulls at a deviation of 2.405 x f; 999 Hz gives 2.40 kHz, 500 Hz gives 1.20 kHz, 2079 Hz gives 5.00 kHz, 1248 Hz gives 3.00 kHz. The method: key the radio with the chosen tone, raise the transmit audio level until the carrier disappears on a spectrum display (the Flex panadapter, a public UberSDR receiver, or the station's own monitor page on a second receiver), and the transmit level at that point is calibrated for that deviation; back it off in proportion for the deviation the mode wants (an AFSK1200 station on a 12.5 kHz channel wants about 2.5 to 3 kHz). The shape when it comes: the single-tone option of the TX test button gains a deviation preset menu with those four pairs and the target deviation shown, the same bounded keying and the same rules as the two-tone test; the daemon cannot see the carrier null itself unless it also has a receiver on the channel (#15 or a second Flex slice, #13), in which case it could read the null off its own waterfall and say the level. The FM radio kind this needed (#413) arrived in PR #428: `"sideband": "fm"` is a third kind beside the two sidebands, so the page, the planner and a Flex FM slice all know the radio is FM, and the page labels its audio as audio.

Shipped in PR #415: an operator-page control (never public), `POST /api/txtest`, and `--two-tone <seconds>` / `--tone <hz> <seconds>`, all through one runner - 700+1900 Hz or a single tone with the four FM presets, queued on the channel like a frame so it defers to carrier sense and to an ARQ hold, at the modulators' own 0.8 peak, default 5 s and capped at 30 s (`txTest`, CONFIG.md), refused when no `ptt` is configured, journalled in ASCII and logged as a `tx-test` transmission that reaches the public monitor of an uplinked station. The presets ship without waiting for #413, labelled as FM. Needs Tom's bench: PTT on real hardware, the tones and their level on a scope or monitor receiver, a real Bessel null at the four tones, and a 30 s key-down into a real PA.

### #19 - More than one modem on one machine: a systemd template unit *(delivered, this PR)*
Tom, 2026-09-14: the package installs one unit, so one modem per machine without hand-written units. Shipped: `pdn-soundmodem@.service` beside the plain unit, the plain unit with `%i` in the two paths (`/etc/pdn-soundmodem/NAME.json`, `StateDirectory=pdn-soundmodem/NAME`) and nothing else changed; the daemon's default `frameLog`, `survey` and `rawCapture` paths now follow `$STATE_DIRECTORY` (`StateDirectory.cs`), which is what made the template safe, because two instances that both enabled the frame log had one absolute default path and would have been two writers on one SQLite file. Instances are the operator's: never enabled by the package, restarted on upgrade, stopped on remove, un-enabled on purge with their config files kept. `test-deb.sh` runs an instance beside the plain unit under real systemd and checks the two frame logs are two files. Ports, device and PTT line are per file and unchecked across processes on purpose: the bind failure already names the port in the journal. Documented in INSTALL.md and `docs/reference/files.md`.
...
2. **FreeDV datac HF-loop validation** (#4) - *needs an HF rig, or the Flex path.* Follow
   [`docs/dev/archive/freedv-hf-loop.md`](archive/freedv-hf-loop.md) (rig recipe, the 8 radio-only unknowns, drive
