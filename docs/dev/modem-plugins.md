...
Status: current as of 2026-09-17. Describes how the daemon loads a modem that is not in this repository: the `IModemPlugin` contract, the isolated load context, and how a plugin mode reaches the catalogue. The contract, the loader and the registry are `IModemPlugin.cs`, `ModemPluginLoader.cs`, `ModemPluginRegistry.cs` and `ModemCatalog.cs` under `src/Packet.SoundModem/Modems/`; the plugins themselves live in repositories of their own. The operator's half is [reference/config.md](../reference/config.md#modemplugins).
...
builds one, the daemon streams audio into it, KISS carries frames out. A plugin therefore supplies two things: an `IModem` implementation, and a way to name and construct it.
...
answer every question the catalogue is asked, as a built-in one does.

It is a **separate type** from the catalogue's own private descriptor rather than that
...
failure, which is named here because it costs an afternoon every time.
...
- **No versioned plugin ABI, yet.** The contract is a normal .NET interface and a plugin built
  against a different `Packet.SoundModem` version may fail to load. The daemon should say
...
`tests/Packet.SoundModem.SamplePlugin` is a trivial modem built as a separate
...
its own copy of `Packet.SoundModem.dll` - which is what a normal build does, and is the
...
Built on 2026-08-09 in four steps: the contract, `ModemDescriptor` and the `plugin:mode` naming; the loader, with shared-contract resolution and explicit paths from the config; the in-repo sample plugin and its tests; and the `modemPlugins` config with its reference entry. The OFDM-FM plugin that prompted the mechanism lives outside this repository and is not tracked here.
