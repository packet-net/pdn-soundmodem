...
Status: design record as of 2026-09-17. Describes the App D 3 kHz serial-tone modem as it was designed on 2026-07-16, before any of it was written, and it is cited from the code for the reasoning behind particular constants. The modem was built and is in this repository at `src/Packet.SoundModem/Ms110d/`, shipping as the `ms110d-wn*` modes; the phases this document plans have both closed, and the closeouts, the later Poor-gate programme and all the evidence are in [../archive/ms110d](../archive/ms110d). Where this document and the code differ, the code and [../mode-validation.md](../mode-validation.md) are the record. Two neighbours it cites as in-tree have left: the LDPC decoder (`Fec/Ldpc`) and the OFDM demodulator moved out on 2026-07-18 and are now the M0LTE.FecLdpc and M0LTE.Ofdm packages.
...
**Tables provenance.** Every load-bearing table value cites `docs/dev/ms110d/tables/` - transcribed **twice independently** (branches `ms110d-tables-a`/`-b`, 2026-07-16), diffed with **zero value conflicts**, plus machine self-checks (constellation symmetries, puncture-mask ones-counts vs D-XLIX rates, the printed WID-0 scrambler regenerating its printed first-32 sequence) - see `docs/dev/ms110d/README.md`. Values *not* yet under that discipline are listed in the ledger (§8) and are **provisional until formally dual-transcribed**; none may freeze into shipped code before its ledger gate clears.
...
Ship a pure-managed C# App D **3 kHz serial-tone** modem: 2400 Bd single-carrier on an 1800 Hz sub-carrier, autobaud preamble, probe-trained DFE, tail-biting convolutional FEC - waveform numbers WN0-6 + 13 in Phase A (75-3200 bps), 8PSK/QAM later (§6). App D is the public counterpart of RESTRICTED STANAG 5069 (`docs/dev/ms110d/README.md`).

### 1.2 What stands in for the oracle

Unlike FreeDV (libcodec2) and ARDOP (ardopcf), **no open App D implementation exists** (verified scoping, `docs/dev/ms110d/README.md`) and no public off-air recordings were found (searched; unknown stated as unknown). The evidence base is therefore built, not borrowed:
...
public static class Ms110dTables {          // constants generated from docs/dev/ms110d/tables/*
...
>
> **Updated later the same day (#67 closure work):** the coverage additions landed - a clock-skew rig (windowed-sinc resample in `Ms110dClockSkewTests`) shows **±50 ppm passes bit-exact on WN2/WN5/WN6** with breaking points at ±700 ppm (~4 s bursts) / ±300-400 ppm (~11 s bursts), so the paragraph's ±50 ppm figure is MET on the tested geometries (tolerance is burst-duration-dependent; longer transmissions unmeasured); **hermetic ±75 Hz CFO passes all four modulation families**; the WN×interleaver×K matrix now covers every distinct (size, increment) cell of D-XXXVII/D-LI (23 pinned rows; all 31 previously-missing combos verified decoding during calibration); WN7/8 joined the interleaver permutation check. Still open from the original list: data-phase late entry, positive EOT-assisted termination, every-super-frame-offset cold start.

**Rung 2 - Watterson channel simulator (the acceptance instrument).** New `tests/…/Channel/WattersonChannel.cs` per Appendix E: ≥4× symbol rate (9600), tapped delay line, per-tap independent complex-Gaussian gains filtered to a Gaussian Doppler spectrum (E.5.3/E.5.4, doc pp.248-249), calibrated SNR in 3 kHz noise bandwidth. "Poor" = ITU-R F.1487 Mid-Latitude Disturbed: 2 independent equal-power Rayleigh paths, 2 ms, 1 Hz two-sigma fade (D.6.1, doc p.240). Self-test pins measured Doppler spread + SNR (house pattern, `docs/dev/archive/ofdm-design.md` §8.6); cross-check against codec2's MPP settings on a FreeDV mode whose published performance we already reproduce. Pass bars = `tables/d6x-ber-masks.csv` (coded BER ≤ 1.0E-5, Long interleaver, 20-super-frame preamble, D.6.1):
...
| **B - 8PSK/16QAM + RLS** | WN7-8 (D-VI map, `tables/d07`); probe-directed RLS (λ = 0.995) replacing NLMS as tracking loop; QAM amplitude reference | **D-LXIV at mask (no allowance), AWGN + Poor, WN0-8+13**; Phase A regressions green - **CLOSED 2026-07-27** ([phase-b-closeout.md](../archive/ms110d/phase-b-closeout.md)): AWGN 10/10 zero-error; Poor hard-gated 8 of 10 (WN0-6+13, both seed families, §5.3 full budgets); WN7/WN8 close **measured-only** under the plan §4 escalation rule with written current-binary verdicts (WN7 2.56E-5/1.48E-5 - the waveform's fade-lottery floor, all lever classes measured dead; WN8 coin-flip behind a 25-92× true-label model ceiling + an uncrossable bootstrap basin - needs waveform-processing redesign, Phase C-class scope) | RLS vs NLMS A/B report - **delivered** ([rls-vs-nlms-report.md](../archive/ms110d/rls-vs-nlms-report.md)) |
...
4. **No off-air ground truth (permanent until Rung 3).** Self-consistent ≠ correct. The claim stays "spec-faithful + mask-passing", never "interop-proven", in mode docs and `docs/dev/plan.md`, until a capture or live counterpart exists.
...
> `docs/dev/ms110d/README.md` § Ledger clearance and `tables/ledger-transcription-notes-{a,b}.md`.
