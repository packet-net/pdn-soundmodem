...
| **B - 8PSK/16QAM + RLS** | WN7-8 (D-VI map, `tables/d07`); probe-directed RLS (λ = 0.995) replacing NLMS as tracking loop; QAM amplitude reference | **D-LXIV at mask (no allowance), AWGN + Poor, WN0-8+13**; Phase A regressions green - **CLOSED 2026-07-27** ([phase-b-closeout.md](phase-b-closeout.md)): AWGN 10/10 zero-error; Poor hard-gated 8 of 10 (WN0-6+13, both seed families, §5.3 full budgets); WN7/WN8 close **measured-only** under the plan §4 escalation rule with written current-binary verdicts (WN7 2.56E-5/1.48E-5 - the waveform's fade-lottery floor, all lever classes measured dead; WN8 coin-flip behind a 25-92× true-label model ceiling + an uncrossable bootstrap basin - needs waveform-processing redesign, Phase C-class scope) | RLS vs NLMS A/B report - **delivered** ([rls-vs-nlms-report.md](rls-vs-nlms-report.md)) |
