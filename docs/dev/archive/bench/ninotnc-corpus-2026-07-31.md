...
## Head-to-head: the NinoTNC decoding its own corpus through the replay chain (2026-08-01)

Replay path finally proven (a crossed test lead had broken it; wiring now verified by continuity: CM108 OUT↔RXA, IN↔TXA). Every corpus file was replayed into the TNC (switch 2 at 1x, Speaker 13/37 ≈ −24 dB) with a hygienic harness - **no serial traffic while audio plays** (GETALL polling during replay starves the TNC's modem loop), **input flushed before every counter read** (stale queued diagnostic dumps otherwise report old counts), and **mode-set verified with retries** (SETHW silently failing to apply manufactured phantom 0/3 rows for fsk9600 until enforced). Harness: `~/replay_verify.py` on studybox.

Result - TNC side, on audio our receiver decodes **45/45** from file:

- **39/45 solid 3/3**: every AFSK1200/300-IL2P variant, all PSK modes, fsk4800-il2p, fsk9600-il2p, fsk9600 AX.25, qpsk3600 - parity with us on those.
- **c4fsk9600 and c4fsk19200: genuinely marginal (~0-2 of 3 per file, repeatable)** - the TNC cannot reliably decode its own C4FSK transmissions through this chain, where our receiver decodes the identical recordings 3/3. Fair caveat: the replay chain (CM108 DAC → double AC coupling → attenuation) is an extra analog hop the file-fed receiver does not suffer, and it specifically punishes amplitude-coded 4-level modes.
- **1100 (300 AFSK AX.25): 0/3 at every tier with mode-set verified**, while 1101/1110 - the same tones under IL2P - decode 3/3. Second observed oddity of this mode on N9600A4 v3.44 (it also refuses to transmit ~253 B AX.25 frames).

Bottom line for the "match the NinoTNC" goal: on this 45-file corpus there is no cell where the TNC decodes and we do not; the two failing-file mystery modes (c4fsk txd50/txd120) that opened this investigation are ones the reference hardware itself cannot copy through the bench chain.

**Open observation - mode 1100 replay (2026-08-01 closed-loop test).** A fresh TNC transmission recorded and immediately replayed decodes fine in 1100 at every corpus frame length (75/131/183 B info), so the mode's RX and the replay chain are healthy - yet the 2026-07-31 corpus 1100 recordings fail replay at every level, despite pdn-soundmodem copying them 3/3. Something in those specific recordings (captured before today's rig handling; TX-deviation pot position that day is an uncontrolled variable) is rejected by the TNC but tolerated by us. First-pass tone comparison was inconclusive (different symbol content under the analysis window). c4fsk closed-loop, by contrast, is marginal even fresh (1/3) - that mode's limit is the chain itself.
