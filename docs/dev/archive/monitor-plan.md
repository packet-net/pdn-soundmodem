...
- a tag on every journal line it writes, the slug, so fifty stations in one journal are readable: `m9psy-1: ubersdr: live, 2 viewers: ...`. Flavour A writes no tag, so its journal lines are byte-identical to today's. Since #409 the phase, the viewer count and, on a wait entered with nobody watching, the words `retrying for nobody` are on every ubersdr line an on-demand receiver's own input writes, including the session's own stream-ended, reconnected and backing-off lines, which used to carry no count at all. The daemon's `ubersdr:` start-up lines (the receiver description, the session limit, the refusal notice) are the daemon's own and carry no count;
- one line of its own when a page stops answering: the waterfall server asks every open page every 20 s whether it is still there and stops counting one that has said nothing for 60 s, writing `m9psy-1: page: viewer dropped, no reply for 60 s, 0 viewers` as it goes (#411). That deadline is the whole of the viewer count's honesty: a socket whose browser has gone does not always close, and behind a Cloudflare tunnel it never does, because this process's TCP peer is a healthy local `cloudflared` rather than the browser. Before it, a phone going to sleep held the count above zero for ever, so the linger never started and the receiver was retried at its 300 s cap all night for nobody (#409). A tab still holding a page from before that release cannot answer, so it is dropped and reconnects once a minute until somebody reloads it, and no server-side change can reach it; from that release on the page checks the version the server announces on every reconnect and reloads itself once, so the same thing cannot happen at the next upgrade;
...
- One row per receiver a visitor can pick: callsign, name, location, `snr_0_30_mhz` as a signal figure where the directory gives one, free slots as `available_clients` of `max_clients`, and a link to `public_url` for the receiver's own page. The row itself links to `/r/<slug>/`. Nothing anywhere on the page says what a receiver is doing or who else is watching it: no column, no viewer count, no tint on a row that has one, and no total across the top. See section 8, 2026-09-04.
- A receiver that cannot be picked is not on the page at all. `/api/instances` still carries it, with `offered` false and `why` saying whether it is full or has gone from the directory, and the page draws neither the row nor the reason; it returns by itself at the next refresh. The count across the top still says how many are not available, so a list shorter than the directory's does not read as receivers having quietly disappeared.
- Sorted by callsign, and by nothing else. Nothing on the page would explain any other order.
...
- Says the awkward things plainly, in the two places that are about the site rather than about one receiver: "the receiver directory is unreachable, this list is from 19:42" across the top, and, when there is nothing at all to pick, that every receiver is full or gone from the directory and that nothing needs doing here. A receiver's own trouble - its daily allowance for this monitor spent, a station not working just now - is said on that receiver's page, in the status chip, which is where the visitor who picked it is.
...
- **The per-address daily allowance now applies per receiver**, and this deployment has one egress address. Fifty receivers means fifty independent allowances, each of which this site can exhaust on its own. The receiver's own page says so in its status chip, where the visitor who picked it is; the picker has said nothing per receiver since 2026-09-04 (section 8). Nobody should have to guess why a receiver went quiet.
...
Tom again, on the picker's column of per-receiver state - "free", "2 watching", "connecting", "just left": "get rid of the column altogether- this is a fact the viewer doesn't care about." And, on what then becomes of the rows it was carrying: the picker lists only the receivers a visitor can pick, so one that is full or has left the directory is simply absent until a refresh brings it back.

Taken as: the column, the header cell over it, the live-first order it was the only explanation for, and the rows nobody can act on all go; every row that is left is a link. The two quieter ways the page said the same thing go with it, on the same reasoning - the faint tint on a row somebody was watching, and the ", 2 being watched" on the end of the count across the top. Who else is here is not a fact a visitor came for, and a page should not say it three ways having been told not to say it once. This supersedes the picker half of "Filtered means not listed; full means listed and not offered" below, and of three others there that quote a sentence the picker used to put in a row - the retry line, the fault line, the daily allowance line. `/api/instances` is untouched and still carries `state`, `viewers`, `why` and `status`; the receiver's own page still says all of it in its status chip, which is the only place a visitor who has picked one will be looking. What is lost from the picker, and worth naming rather than discovering: a visitor can no longer see from the front page that a receiver's daily allowance for this monitor is spent, or that a station is not working just now - they see it on the receiver's page, one click away.
