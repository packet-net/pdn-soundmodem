This is the amendment log of docs/plan.md as it stood on 2026-09-10. It is closed and frozen: nothing below this note is edited.
The current plan record is docs/dev/plan.md.
...
**All ALSA mixer work now finishes before the PCM is opened, and that is a rule rather than a tidy-up.** Measured on radio1 with the pass inside that window, 10 runs dead out of 13, against 12 of 12 alive for the same source doing two fewer reads per control, and 10 of 10 alive with it moved ahead of the open. Written into CLAUDE.md, pinned by `StartUpOrderTests` and recorded with the numbers in [roadmap.md](roadmap.md) #17. The mechanism recorded with it at the time (a control transfer colliding with the first read's URB submission) was a guess; the strace of the qpsk3600 start-up failure shows an overrun of the 120 ms capture buffer by a stream that was started at `snd_pcm_open` and then not read for 150 ms, and a recovery that read again 5 ms after its prepare and got `-EIO`. Anything that delays the first reads does it, so the reorder shortened the gap rather than closing it; the buffer and the recovery are the fix (see #17).
...
**A station is an ordinary station over a socket.** `RelayStation` is a `SoundModemChannel` at the rate the station says it is relaying at, **with no modems added at all**, a `Station` over an `UplinkAudioInput` fed from the WebSocket, and a `WaterfallWebServer.Routed` under `/r/<slug>/` exactly as a receiver's is. So the waterfall, the band overlays, the browser audio, the links panel, the frame log and the page all come out of code that already runs in production, and the only new thing is where the samples came from. The site runs no modem for a relayed station and never will: what the page lists is what that operator's daemon decoded, with their modes, their diversity settings and their dial, pushed in through Phase 1's `PushFrame`. The links panel is folded here from the frames' own bytes rather than sent as a summary, so there is one implementation of a link card and the fold outlives the station going off air. A frame waits until the audio that carried it has been painted, because listing it on arrival would tag it five to twelve lines above its own burst.

**The token is the whole credential, and it is checked before anything is allocated.** The site issues it - `pdn-soundmodem --uplink-token` mints 256 bits and prints the token and its SHA-256 together, writing neither anywhere - and holds only the hash, compared constant-time over the raw bytes as the config API's key check is. The callsign is bound to the token, so one station cannot claim another's page; the slug never comes off the wire at all, so no station can ask for one. A second connection on one token closes the first, because a station whose socket has half-closed must not be locked out by its own ghost. A token this site never issued is held for a second and counted in the journal, at most a line a minute.

**Everything a station sends is somebody else's writing.** Lengths are capped at the boundary and over a cap is a refused connection or a dropped message rather than a truncation; a `site` URL goes through the same absolute-http-or-https check the receiver directory's `public_url` does; everything reaching the journal is flattened to ASCII; the jitter buffer is bounded and drops the oldest. An oversized message, an audio block that is not the length the station's own hello declared, or a sustained rate over twice what it declared closes the connection, names the station in one line, and refuses that token for a minute. Nothing a station sends is forwarded: every message is parsed into typed fields and the site re-serialises its own, so there is no path by which its bytes reach a browser unexamined.

**The picker is one list with two categories.** Tom: "One list with two categories." The five-cell row keeps its shape and its sort by callsign, with a small word in the first cell saying which kind it is; a station's row shows its callsign, operator, location, radio and modes, and the two figures it has no honest answer for are blank rather than borrowed. `/api/instances` gains `kind`, and the receiver page gains a second credit sentence chosen by a word from the server rather than written by it - the sentence carries an anchor around an escaped name, so a server-supplied sentence would be a second way for a third party's words to arrive as markup.

**About 1.4 MB per relayed station**, against 31 MB for a receiver's: 89.4 MB with none, 92.1 with one, 96.2 with five, and 1.0 MB per station under `DOTNET_GCConserveMemory=9`. It is the demodulators, as it was for the receivers - a relayed station runs none of the twenty. Fifty of them would be 70 MB against the 1.6 GB fifty receivers cost, so nothing about the container's sizing changes.

**Nothing changes for a receiver, or for a monitor with no `uplinks`.** Proved the way PR #395's was: the CT 146 configuration run against a frozen local directory on `main` and on the branch, stdout and stderr byte-identical, `/api/instances` identical across all 47 receiver rows field for field save the one added `kind: "receiver"`, and `/robots.txt` identical. The two page files differ by exactly the edits above and their version hashes.

Two things could not be done from this sandbox and are not claimed: the live two-process smoke, which needs Phase 2's client and is deferred to when both have merged, and looking at the picker in a real browser, which has neither a browser nor internet. The page evidence is the Node probes running the shipping JavaScript in real V8.

### 2026-09-04 (later) - the picker loses its state column, and lists only the receivers you can pick

**The column goes.** Tom, on the picker's column that said "free", "2 watching", "connecting" or "just left" for each receiver: "get rid of the column altogether- this is a fact the viewer doesn't care about." Gone with it: the header cell over it, the `stateOf` and `viewersText` functions behind it, the CSS that coloured it, and the live-first order, which that column was the only thing on the page that explained. The list is by callsign now, and by nothing else. The other two places the page said who else was here go on the same reasoning: the faint tint on a row being watched, and the ", 2 being watched" on the end of the summary. The page now says nothing at all about the other visitors.
...
Phase 3 of [docs/monitor-plan.md](monitor-plan.md): the `monitor` config section, the directory client behind it, the host that owns the port, and the picker page. Phase 1 gave the daemon a `Station` type so a process could hold more than one; Phase 2 let a waterfall page be served under a path prefix behind one listener; this is the flavour that uses both. Same binary, same package, same tests, and the single-station flavour proven byte-for-byte unchanged by running the CT 146 configuration against M9PSY-1 on `main` and on the branch and diffing both output streams.

**A `monitor` section instead of a `device`.** It names the directory to fetch the list of receivers from, how often, how long each receiver's session lingers after its last viewer leaves, an allow and a deny list of hosts, and the modems every receiver is given. `monitor` and `device` are mutually exclusive and the daemon says so, naming both. There is no KISS, no PTT, no transmitter, no config API and no survey in this flavour, and none of them is reachable on its port: a monitor is a display. `waterfall.public` is forced true rather than checked, because a picker that lists other people's receivers and invites anyone to watch one is a page for strangers by definition, and `frameLog.path` becomes a directory holding one `frames-<slug>.db` per receiver.

**The list is the UberSDR project's own, filtered and remembered.** A receiver that is offline, will not offer the IQ mode this monitor asks for, has no antenna connected or reports a tuning range that cannot reach the window the modems occupy is not listed; one with no free listener slot is listed and shown as full, because that is a receiver a visitor may well come back to. `deny` beats `allow` and both match the directory's own `host`: that is the mechanism by which an operator who asks not to be listed is not listed, so it is not a convenience and it is tested. A failed fetch keeps the last good list, writes one line per outage rather than one per refresh, and the picker says how old the list it is showing is; a receiver that leaves the directory keeps its page for as long as anybody is on it and simply stops being offered. Each receiver's page is served under a slug derived from its host - `m9psy-1`, `reading-ubersdr-m0lte-uk` - which is the only field the directory guarantees unique, and which does not change when an unrelated receiver appears.

**One session per receiver, however many people are watching it.** This is the promise the design makes to the people whose antennas these are, and it is a test rather than a hope: two real WebSockets on one routed station, one `OpenAsync`. The fan-out is in the daemon - ten visitors on one receiver are ten browsers on one page, which is one viewer count, which is one session, dropped sixty seconds after the last of them leaves. A receiver nobody has picked costs its operator nothing at all: the first request for its page builds its channel, modems, frame log and page, and the receiver itself is not contacted until a browser actually attaches, so a crawler cannot cost anybody a session either. Smoked against M9PSY-1 and Reading together: two sessions, one browser leaves, that receiver's session drops after the linger and the other one does not.

**The memory figure came out an order of magnitude above the guess, and it is the modems.** About 31 MB per station for the three-modem 40 m band plan, linear from two to thirty, against 86 MB for the process with nothing picked - so fifty receivers all picked is about 1.6 GB. Almost all of it is the frequency-diversity banks: `afsk300-il2pc` runs 11 decoder branches by default and `bpsk300` runs 9, and `"offsetPairs": 0` on both brings it to 3.5 MB per station. The banks are what took the live 40 m slot from 3 of 13 frames to 13 of 13, so they are not free to give up, and the trade-off is Tom's rather than the implementer's; the measurement and both sides of it are written into section 5 of the monitor plan.

**Shared construction rather than a second copy.** The modem-building loop, the mode checks and the DSP-rate decision, the frame log's open and wiring, the per-frame journal line and the id-beacon ghosts moved out of `Program.cs`'s top-level statements into a `StationFactory` both flavours call, in the same order and writing the same lines. A forked modem loop would have been two answers to what a config actually runs, and the difference would have shown up as a receiver decoding something other than what was asked for with nothing anywhere saying so.

Also in there: the static `HttpClient` behind the UberSDR pre-flight gets a five-minute `PooledConnectionLifetime`, because directory-supplied hostnames are exactly the case the default infinite one gets wrong; `OnDemandUberSdrInput.Refused` makes the reconnect ladder's own knowledge of a refusal readable, so a site listing fifty receivers can say which of them is refusing it rather than which is broken; and `WaterfallWebServer.SetRadioStatus` gains the null guard its neighbour `SetReceiver` has always had, having thrown when called before `Start`.
