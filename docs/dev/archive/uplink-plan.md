...
| `frame` | text | `BroadcastFrame` :1349 | per decode and per transmission | `line`, `sub`, `mode`, `from`, `to`, `lenBytes`, `snrDb`, `burstLines`, `offsetHz`, `corrected`, `crc`, `id`, `tx`, `txTrimHz`, `why`, `il2p`, `hex`, `plain`, `monitorOnly`, `peakDbFs`, `clipped`, `level` |
...
**Amended 2026-09-05 (v0.56.2, PRs #410 and #412).** Following up the receiver churn from the soak (issue #409): the site now writes the viewer count into every on-demand journal line (#410), and that logging showed the cause. A page's socket that died without a close handshake (behind the tunnel the daemon's TCP peer is cloudflared, not the browser) was never noticed, so the receiver's viewer count stuck at 1 and the site kept retrying a failed receiver for nobody (issue #411). Fix (#412): the waterfall server pings every open page every 20 s, the page answers, and a page that has said nothing for 60 s is dropped with a journal line `page: viewer dropped, no reply for 60 s, N viewers`; the same applies on a station's own operator page and to the pages the site serves for a relayed station. Deployed to the site (CT 146) and the node (`~/update.sh`, Tom's standing permission) at 10:06 UTC on 2026-09-05; the node's publish line said `live at https://monitor.ukpacketradio.network/r/gb7rdg/` within two seconds and both KISS hosts were back within 13 s. In the first 12 minutes CT2HMR stayed idle and EI4HQ showed one browser tab from before the upgrade being dropped and reconnecting every 65 s (the old page cannot answer pings; a tab opened on 0.56.2 or later reloads itself once on the next upgrade instead). CI on the merge commit needed one rerun for an unrelated flake, filed as #414. Overnight read of both journals to follow.
...
- [~] **Phase 4** - release, monitor config, GB7RDG-2 opted in, validated live, the keyup level judged and the upstream rate measured, soaked, recorded, announced. Needs 2 and 3. Release v0.56.0 (2026-09-04); live, rate measured, memory recorded; id changed to GB7RDG; v0.56.1 (2026-09-04) with #401 #402 #403 deployed to the site; soaked overnight 2026-09-04/05 (three 1 s drops, two of them our restarts); v0.56.2 (2026-09-05) with the viewer-count logging and the page keep-alive (#409, #411) deployed to the site and the node, overnight read to follow; keyup level and announcement are Tom's. PRs: #399, #405, #406, #407, #410, #412
