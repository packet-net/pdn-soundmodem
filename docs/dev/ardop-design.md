...
*Status (2026-07-17): software side __done__ - `Ardop/Host/` (`ArdopHostTnc` + `ArdopHostServer`) implements the full ardopcf command set byte-for-byte (107-command transcript diff against a live ardopcf 1.0.4.1.3: identical bar VERSION; quirks ported: the `not recoginized` spelling, NEWSTATE's trailing space, PROTOCOLMODE accept-anything, PING-in-RXO double reply; boot protocol mode is ARQ - `ardopmain()` overrides the FEC variable initializer). RXO was scoped in and shipped (`ArdopDemodulator.RxoMode` - `RxoComputeDecodeDistance`/`RxoDecodeSessionID` port). Daemon: `--ardop <port>` / `"ardop"` config section (data port = port+1); dedicated-channel policy enforced (exclusive with modems/paging, CSMA persistence forced to 255, 20 ms capture blocks; the 500 ms capture buffer was ARDOP-only until PR #422 made it every station's, so it is no longer part of this policy). Rung 4 green: real Pat 1.0.0 exchanged a message end-to-end (Pat→our daemon→aloop→ardopcf→Pat, full B2F, message landed in the far mailbox), scripted-Pat full-stack sessions byte-exact in both roles, and the RXO monitor decoded a live third-party ardopcf↔ardopcf session (dsnoop-tapped aloop channel) - `ArdopHostTncTests`/`ArdopHostServerTests`/`ArdopHostLiveTests`. Remaining: Rung 5, the GB7RDG on-air acceptance.*
