...
- `frame`, text: the display fields of a `frame` message, plus `at` (UTC, ISO 8601), plus `raw` (base64 of the AX.25 bytes, absent on an ident ghost or a reported frame that had none). Always, whether or not anybody is watching. The one display field that does not cross is `level`, the too-loud/too-quiet verdict: `peakDbFs` and `clipped` are the measurements and `level` is a rule applied to them, so the monitor applies its own rather than being told one - the thresholds have one definition, and a relayed row is badged by the same code as a local one.
