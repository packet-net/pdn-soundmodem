...
2. **There are two inbound message types, and between them they carry a slug, a URL and one integer.** `welcome`, once, which does nothing but let publishing begin and put a line in the journal; and `demand`, which carries a viewer count. The reader's `switch` has those two cases and a default that increments a counter and drops the message, and a second `welcome` is counted and dropped rather than restarting anything. A message whose `type` is not a string, or whose `viewers` is not a number, is dropped as well rather than throwing, so a monitor that grows a field or types one wrongly cannot end a station's session. A test asserts that a `config`-shaped message, a KISS-shaped message, a transmit request and every badly typed shape all do nothing, and that not one of them costs a reconnect.
...
- **Memory** should be small and needs measuring rather than asserting. A relayed station has no demodulators, so none of the 31 MB per station the monitor plan measured applies; what is left is a channel with no modems, a waterfall server, a frame log, a link observer and a page, which the same ablation put at about 1.5 MB. Phase 3 measures it with 0, 1 and 5 relayed stations and writes the number back into this section, the way the monitor plan's Phase 3 did.
