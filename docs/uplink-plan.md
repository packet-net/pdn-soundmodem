...
2. **There is one inbound message type, and it carries one integer.** `demand`, and a viewer count. The reader's `switch` has one case and a default that increments a counter and drops the message. A test asserts that a `config`-shaped message, a KISS-shaped message and a transmit request all do nothing.
