...
| `Ardop/*` (frame codec, 4FSK modulator + demodulator, FEC mode) | **ardopcf lineage (Rick Muething KN6KB / John Wiseman G8BPQ / Peter LaRue AI7YN), MIT** — v1.0.4.1.3, git `a7c9228`, ports with per-file `file:line` citations (docs/ardop-design.md §8.2 policy). The ARDOP Specification Rev 2.0 (2017-11-27, released to the public domain) is cited alongside, but the operative wire format is the code's: the nonstandard CRC-16 (0x8810 constant, LSB injection — spec *says* CCITT), the frame-type parity quirk (both parity symbols from the plain type byte), the frame geometry from `FrameInfo`, and the TX integer sample templates transcribed verbatim from `ardopSampleArrays.c` (`ArdopTxTemplates.g.cs`). The RX chain (leader search, envelope-correlator framing, phase-reversal sync, minimal-distance type decode, Goertzel 4FSK demod, Memory-ARQ averaging) and the TX comb-resonator filter are behavioural ports of `SoundInput.c`/`Modulate.c`. Reed-Solomon reuses this repo's `Fec/ReedSolomon` — same GF(2^8) 0x11D field and α¹… generator as ardopcf's `lib/rockliff/rrs.c` (Simon Rockliff, permissive) — through the Rockliff byte-reversed wire layout (`ArdopFrameCodec.RsAppend`), **established empirically** against rrs.c-generated vectors. Validated bidirectionally against ardopcf itself: its TXFRAME audio decodes here payload-exact and our audio decodes in its `--decodewav` (samples/ardop/). The ARQ session engine (`Ardop/Arq/*`) is a behavioural port of `ARQ.c` (state machine, `IRSNegotiateBW`, `Gearshift_9` with its threshold tables, `ComputeInterFrameInterval`, DISC/END teardown incl. the END-after-`InitializeConnection` session-ID quirk — ported, not fixed) plus the timer/poll scaffolding of `ARDOPC.c` (`CheckTimers`/`MainPoll`) and the IRStoISS shortcuts of `SoundInput.c`; validated by live ARQ sessions ours↔ardopcf over snd-aloop, both roles (tests/…/ArdopArdopcfLiveSessionTests.cs). |
