...
// graph either side of it, and a PTT line over Web Serial. No waterfall, no config API, no
// KISS - what comes out is raw AX.25 frames, which is what @packet-net/ax25 takes.
//
...
export class SoundModem {
   ...
   */
  static async load(frameworkUrl = './_framework/dotnet.js') {
  ...
  /** The audio rate the graph is running at, once open. */
  get sampleRate() { return this.#context?.sampleRate ?? 0 }

  /** True while this station is keyed. */
  ...
  async open({ mode = 'afsk1200', inputDeviceId, outputDeviceId, ptt = NoPtt, sampleRate = 48000 } = {}) {
    ...
    await this.#context.audioWorklet.addModule(new URL('./sm-rx-worklet.js', import.meta.url))
  ...
  /** True while the demodulator sees a coherent packet signal (the DCD lamp). */
  get carrierDetect() { return !this.#transmitting && this.#exports.CarrierDetect(this.#handle) }

  /** Carrier sense for @packet-net/ax25: busy while anything, including us, is on channel. */
  channelBusy() {
