...
/// write a hot loop, but it is obviously correct, and correctness is what a waveform whose
/// parameters are still moving should be spending first.</para>
/// </remarks>
public static class OfdmFmMapper
{
    ...
    /// <summary>
    /// The closest any two distinct points of this constellation ever sit to each other: how far a
    /// received point can drift off its intended one before it is nearer some other symbol value.
    /// Every table here is a regular grid or a regular ring, so this is the same for every point in
    /// it and one pass over the table settles it; nothing here is large enough (256 points at most)
    /// for the pairwise search to be worth precomputing.
    /// </summary>
    public static double MinimumDistance(OfdmFmConstellation constellation)
    {
        (float I, float Q)[] points = Points(constellation);
        double nearest = double.MaxValue;
        for (int a = 0; a < points.Length; a++)
        {
            for (int b = a + 1; b < points.Length; b++)
            {
                double di = points[a].I - points[b].I;
                double dq = points[a].Q - points[b].Q;
                nearest = Math.Min(nearest, Math.Sqrt((di * di) + (dq * dq)));
            }
        }
