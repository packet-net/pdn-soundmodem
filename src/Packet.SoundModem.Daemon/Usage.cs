...
/// argument switch and fails if a flag it parses is missing from this text.
/// </summary>
internal static class Usage
{
    ...
    /// <summary>The usage text, with no trailing newline. Every line fits 80 columns.</summary>
    internal static string Text { get; } = $"""
        ...
        Options:
          ...
          --capture-rate HZ       ALSA capture and playback rate (default 48000); it
                                  ...
                                  48000.
          --kiss PORT             The shared KISS TCP port (default 8105); every modem
                                  is on it, addressed by the sub-channel nibble.
          ...
          --ptt SPEC              How the radio is keyed: serial:DEVICE[:rts|:dtr] or
                                  cm108:HIDRAW[:GPIO]. Refused with flex: and ubersdr:
          ...
          --ardop PORT            Start the ARDOP virtual TNC: command port PORT, data
                                  port PORT+1. A modem with "mode": "ardop" is the newer
                                  way to say it, and giving both is refused.
        ...
        the file states them. --modem adds to the file's modems. Every other flag
        overrides the file's matching section: --ardop over "ardop", --paging over
        "paging", --ptt over "ptt", --waterfall and --dial over "waterfall", --flex-*
        over "flex".
